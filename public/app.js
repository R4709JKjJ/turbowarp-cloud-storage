...
(() => {
  ...
  // URL del backend su Render. Impostato in modo assoluto così questo file
  // funziona sia servito dal backend stesso, sia aperto come pagina
  // standalone (es. da file locale o da un altro dominio).
  const API_BASE = 'https://turbowarp-cloud-storage.onrender.com';

  const state = {
    me: null,
    token: localStorage.getItem('filo_token') || null,
  ...
  loginForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      localStorage.setItem('filo_token', data.token);
      state.me = data.user;
      enterApp();
    } catch (err) {
  ...
  async function tryResumeSession() {
    if (!state.token) return; // nessun token salvato: resta sulla schermata di login
    try {
      const data = await api('/auth/me');
      state.me = data.user;
      enterApp();
    } catch (_) {
      // token assente/scaduto: lo rimuoviamo e restiamo sulla schermata di login
      state.token = null;
