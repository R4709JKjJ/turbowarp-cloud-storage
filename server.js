 ...
 *
 * NOTA — TOKEN INVECE DI COOKIE
 * L'autenticazione usa un token opaco (header "Authorization: Bearer ...")
 * anziché un cookie di sessione, perché il frontend può essere aperto da
 * un'origine diversa dal server (es. un file HTML standalone): i browser
 * moderni bloccano di default i cookie di terze parti in quello scenario,
 * mentre un token esplicito nell'header funziona sempre. Il token è tenuto
 * in memoria e si invalida ad ogni riavvio del servizio.
 *
...
ensureSchema(); // deve girare prima di preparare qualsiasi query, altrimenti le tabelle non esistono ancora
...
// ----------------------------------------------------------------------
// Autenticazione a token (invece dei cookie di sessione)
// ----------------------------------------------------------------------
// I cookie cross-site vengono bloccati di default dai browser moderni
// quando l'HTML è aperto da un'origine diversa da quella del server (es.
// un file HTML standalone che chiama le API su Render). Per questo Filo
// usa un token opaco, generato al login e inviato dal client in ogni
// richiesta tramite l'header "Authorization: Bearer <token>" — un
// meccanismo che non dipende dalle policy sui cookie di terze parti.
// Il token è tenuto in memoria: valido finché il servizio resta attivo,
// invalidato ad ogni riavvio (coerente con la natura "demo" dell'app).
...
function requireAuth(req, res, next) {
  const userId = getUserIdFromToken(extractToken(req));
  if (!userId) {
...
app.post('/api/auth/login', loginLimiter, (req, res) => {
  try {
    ...
    res.json({ token, user: { id: user.id, phone: user.phone_number, displayName: user.display_name } });
  } catch (err) {
...
app.get('/api/auth/me', requireAuth, (req, res) => {
  try {
    const u = stmts.getUserById.get(req.userId);
...
// Il client passa il token ottenuto al login in socket.handshake.auth.token
// (stesso token usato per le richieste REST via header Authorization).
io.use((socket, next) => {
