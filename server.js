 ...
 * Filo — server backend
 * -----------------------------------------------------------------------
 * Express + SQLite (file locale, zero configurazione) + Socket.IO,
 * pensato per il deploy su Render.com come semplice "Web Service"
 * (nessun database esterno da collegare, nessuna variabile d'ambiente
 * obbligatoria).
 *
  ...
  try {
    ...
    if (existing) {
      ...
      const createTx = db.transaction(() => {
        ...
        stmts.insertMember.run(id, meId);
        stmts.insertMember.run(id, contact.id);
        return id;
      });
      conversationId = createTx();
